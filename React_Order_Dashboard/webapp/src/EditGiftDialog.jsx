...
// ------------- Uses same CSS as AddGiftDialog.jsx so classnames still says "addGift" --------------


function EditGiftDialog({ onEdit, gift }) {
  ...
  const [form, setForm] = useState({
    ...
    productionStatus: gift.productionStatus,
    deliveryStatus: gift.deliveryStatus,
  })

  useEffect(() => {
    //console.log('gift changed in Edit Dialog:', gift);
    setForm({
      name: gift.name,
      location: gift.location,
      giftName: gift.giftName,
  ...
  const footer = (
    <div>
      ...
      <Button label="Complete" onClick={handleSubmit} />
    </div>
  )

  return (
    <div className="addGift-dialog">
      <Button className='buttons' label="Edit" onClick={() => setVisible(true)} />
